...
async def set_var(client: Client, message: Message):
    ...
    ALBY = await message.reply("💈 `Memproses!")
    ...
    if await is_heroku():
        ...
        if to_set in heroku_config:
            await ALBY.edit(f"Berhasil Mengubah var {to_set} menjadi {value}")
        else:
            await ALBY.edit(f"Berhasil Menambahkan var {to_set} menjadi {value}")
        heroku_config[to_set] = value
    else:
        path = dotenv.find_dotenv("config.env")
        if not path:
            return await ALBY.edit(".env file not found.")
...
async def varget_(client: Client, message: Message):
    ...
    if await is_heroku():
        ...
        if check_var in heroku_config:
            ...
            return await ALBY.edit(f"Tidak dapat menemukan var {check_var}")
    else:
        path = dotenv.find_dotenv("config.env")
        if not path:
            return await ALBY.edit(".env file not found.")
        ...
        if not output:
            await ALBY.edit(f"Tidak dapat menemukan var {check_var}")
        else:
            return await ALBY.edit(f"<b>{check_var}:</b> <code>{str(output)}</code>")
