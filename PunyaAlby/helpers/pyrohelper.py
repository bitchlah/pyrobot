...
import PunyaAlby.database.welcomedb as ALBYdb
from PunyaAlby.database.afkdb import get_afk_status
from PunyaAlby.database.pmpermitdb import get_approved_users, pm_guard
...
async def welcome_chat(filter, client: Client, message: Message):
    to_welcome = await ALBYdb.get_welcome(str(message.chat.id))
    if to_welcome:
        return True
    else:
        return False
